<div class="table-responsive">
    <table class="table table-shopping">
        ...
        <tr>
            ...
            <th>Изображение</th>
            <th>Заголовок</th>
            <th class="th-description">Первое описание</th>
            <th class="th-description">Второе описание</th>

            <th class="text-right">
{{--                <a href="{{route('admin.services.create')}}" class="btn btn-primary">Создать</a>--}}
            </th>
        </tr>
        </thead>
        ...
        @foreach($items as $item)
            <tr class="draggable" data-sort="{{$item->sort}}" data-id="{{ $item->id }}">
                ...
                <td> {{$item->description_first ?? ''}}</td>
                <td> {{$item->description_second ?? ''}}</td>
                <td class="text-primary text-right">
                    <div class="dropdown menu_drop">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton_1">
                            <a href="{{route('admin.services.edit',$item)}}" class="dropdown-item">️Редактировать</a>
{{--                            <form method="POST" action={{route('admin.services.destroy',$item)}}"" accept-charset="UTF-8"--}}
